...
if [ -z "${RAILWAY_PUBLIC_DOMAIN:-}" ] || [ -z "${RAILWAY_TCP_PROXY_DOMAIN:-}" ] || [ -z "${RAILWAY_TCP_PROXY_PORT:-}" ]; then
  if [ -n "${RAILWAY_TOKEN:-}" ] || [ -n "${RAILWAY_API_TOKEN:-}" ]; then
    # railway_setup.py uses rc=10 to request a restart after infrastructure mutation.
    # Do not let set -e turn that intentional control signal into an opaque startup exit.
    set +e
...
for f in "$D/runtime.json" "$D/state.json" "$D/manifest.json" "$D/runtime-manifest.json" "$D/subscription.txt" "$D/subscription.txt.tmp" "$D/subscription_url.txt" "${XRAY_CONFIG:-$D/config.json}"; do rm -f "$f"; done
...
# Only identity-init.py may initialize identity. It never replaces an existing identity.
python3 /opt/xray/scripts/identity-init.py
UUID_FILE="$D/uuid.txt"; [ -s "$UUID_FILE" ] || { echo "FATAL: persisted UUID missing after identity initialization" >&2; exit 1; }
UUID="$(tr -d '[:space:]' <"$UUID_FILE")"; printf '%s' "$UUID" | grep -Eq '^[0-9a-fA-F-]{16,64}$' || { echo "FATAL: persisted UUID is invalid" >&2; exit 1; }
PRIV_FILE="$D/reality_private_key.txt"; PUB_FILE="$D/reality_public_key.txt"; [ -s "$PRIV_FILE" ] && [ -s "$PUB_FILE" ] || { echo "FATAL: persisted REALITY key files missing" >&2; exit 1; }
PRIVATE_KEY="$(tr -d '[:space:]' <"$PRIV_FILE")"; PUBLIC_KEY="$(tr -d '[:space:]' <"$PUB_FILE")"; [ -n "$PRIVATE_KEY" ] && [ -n "$PUBLIC_KEY" ] || { echo "FATAL: persisted REALITY key files are incomplete" >&2; exit 1; }
TOKEN_FILE="$D/subscription_token.txt"; [ -s "$TOKEN_FILE" ] || { echo "FATAL: persisted subscription token missing" >&2; exit 1; }; TOKEN="$(tr -d '[:space:]' <"$TOKEN_FILE")"; [ -n "$TOKEN" ] || { echo "FATAL: persisted subscription token is empty" >&2; exit 1; }
IDS_FILE="$D/reality_short_ids.json"; [ -s "$IDS_FILE" ] || { echo "FATAL: persisted REALITY short IDs missing" >&2; exit 1; }
python3 - "$IDS_FILE" <<'PY_IDS'
import json,sys
from pathlib import Path
try: obj=json.loads(Path(sys.argv[1]).read_text())
except Exception as exc: raise SystemExit(f"FATAL: persisted REALITY short IDs are invalid: {exc}")
if not isinstance(obj,list) or len(obj)!=3: raise SystemExit("FATAL: persisted REALITY short IDs are invalid")
for item in obj:
    if not isinstance(item,str) or not (2 <= len(item) <= 32): raise SystemExit("FATAL: persisted REALITY short ID length invalid")
...
import socket,sys
try: infos=socket.getaddrinfo(sys.argv[1],int(sys.argv[2]),type=socket.SOCK_STREAM)
...
TCP_PROXY_DNS=UNKNOWN; TCP_PROXY_CONNECTIVITY=UNKNOWN; TCP_PROXY_PROBE_ELAPSED=0; _probe=0
while [ "$_probe" -lt "$TCP_CHECK_SECONDS" ]; do if tcp_proxy_probe; then TCP_PROXY_DNS=READY; TCP_PROXY_CONNECTIVITY=READY; TCP_PROXY_PROBE_ELAPSED="$_probe"; break; fi; TCP_PROXY_DNS=READY; TCP_PROXY_CONNECTIVITY=WAITING; _probe=$((_probe + TCP_CHECK_INTERVAL)); [ "$_probe" -lt "$TCP_CHECK_SECONDS" ] && sleep "$TCP_CHECK_INTERVAL"; done
...
import json,sys
from pathlib import Path
r=json.loads(Path(sys.argv[1]).read_text()); lines=[x.strip() for x in Path(sys.argv[2]).read_text().splitlines() if x.strip()]; expected=int(r.get('nodes',{}).get('count',0)); tcp=r.get('tcp_proxy',{}); public=r.get('public_domain',''); assert expected in (4,5) and len(lines)==expected; assert lines[0].startswith('vless://') and ('@'+public+':443?') in lines[0]; [assertion for assertion in []];
...
PY
xray run -test -config "$XRAY_CONFIG"; xray run -config "$XRAY_CONFIG" & XP=$!
...
if [ "$CF_ENABLED" = "1" ]; then cloudflared tunnel --no-autoupdate --metrics 127.0.0.1:2000 run --token "$CF_TOKEN" >"$D/cloudflared.log" 2>&1 & CFP=$!; i=0; while :; do if python3 -c 'import urllib.request; urllib.request.urlopen("http://127.0.0.1:2000/ready", timeout=1).read()' 2>/dev/null; then echo "CLOUDFLARED_READY=PASS"; break; fi; kill -0 "$CFP" 2>/dev/null || { echo "FATAL: cloudflared exited before readiness" >&2; tail -50 "$D/cloudflared.log" >&2 || true; exit 1; }; i=$((i+1)); [ "$i" -lt "${CLOUDFLARE_READY_TIMEOUT:-45}" ] || { echo "FATAL: cloudflared readiness timeout" >&2; tail -50 "$D/cloudflared.log" >&2 || true; exit 1; }; sleep 1; done; fi
...
wait_port 127.0.0.1 10086 xhttp-http; wait_port 127.0.0.1 10087 raw-reality-vision; wait_port 127.0.0.1 10088 xhttp-reality; wait_port 127.0.0.1 10089 grpc-reality; if [ "$CF_ENABLED" = "1" ]; then wait_port 127.0.0.1 "$CF_PORT_STATE" cloudflare-xhttp-origin; fi
...
if [ "$CF_ENABLED" = "1" ]; then echo "NODE5=VLESS_XHTTP_TLS_CLOUDFLARE"; echo "CLOUDFLARE_TUNNEL=READY"; fi
echo "========================================"
